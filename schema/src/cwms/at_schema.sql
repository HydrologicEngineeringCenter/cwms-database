...
COMMENT ON COLUMN AT_PHYSICAL_LOCATION.SEARCH_DOC          IS 'Denormalized text document used for full-text searches on this location.';
...
CREATE OR REPLACE TRIGGER AT_PHYSICAL_LOCATION_T_SEARCH_DOC
...
BEGIN
   ...
   FROM AT_BASE_LOCATION b
   WHERE b.BASE_LOCATION_CODE = :NEW.BASE_LOCATION_CODE;

   SELECT k.LOCATION_KIND_ID
   INTO l_location_kind_id
   FROM CWMS_LOCATION_KIND k
   WHERE k.LOCATION_KIND_CODE = :NEW.LOCATION_KIND;

   :NEW.SEARCH_DOC :=
      COALESCE(l_base_location_id, '') || ' ' ||
      COALESCE(:NEW.SUB_LOCATION_ID, '') || ' ' ||
      CASE
         WHEN :NEW.SUB_LOCATION_ID IS NOT NULL THEN
            l_base_location_id || '-' || :NEW.SUB_LOCATION_ID || ' '
         ELSE
            ''
         END ||
      COALESCE(:NEW.PUBLIC_NAME, '') || ' ' ||
      COALESCE(:NEW.LONG_NAME, '') || ' ' ||
      COALESCE(:NEW.DESCRIPTION, '') || ' ' ||
      COALESCE(:NEW.MAP_LABEL, '') || ' ' ||
      COALESCE(:NEW.NEAREST_CITY, '') || ' ' ||
      COALESCE(l_location_kind_id, '') || ' ' ||
...
(
   ...
   COMPARISON_UNIT            NUMBER(14)                   NOT NULL,
