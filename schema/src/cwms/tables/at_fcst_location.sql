...
create unique index at_fcst_location_idx2 on at_fcst_location (case when sort_order = -1 then fcst_spec_code end, case when sort_order = -1 then sort_order end);
...
comment on column at_fcst_location.sort_order     is 'The sort order for the location. -1 indicates primary location.';
