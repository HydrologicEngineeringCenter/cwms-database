-- delete from at_clob where id = '/VIEWDOCS/AV_CWMS_TS_ID_NEW';
insert into at_clob values (cwms_seq.nextval, 53, '/VIEWDOCS/AV_CWMS_TS_ID_NEW', null,
'
/**
...
AS
    SELECT db_office_id,
           case when cwms_ts.is_lrts_char(ts_code) = 'F' then cwms_ts_id
                else location_id
                     ||'.'||parameter_id
                     ||'.'||parameter_type_id
           ...
           parameter_id,
           parameter_type_id,
           case when cwms_ts.is_lrts_char(ts_code) = 'F' then interval_id
                else regexp_replace(interval_id, '^~(.+)$', '\1Local')
           end as interval_id,
           duration_id,
           version_id,
           interval,
...
begin
	execute immediate 'grant select on av_cwms_ts_id_new to cwms_user';
exception
...
create or replace public synonym cwms_v_ts_id_new for av_cwms_ts_id_new;