...
--htp.p('Coming Soon... downloading.');


        SELECT 'text/plain' -- mt.media_type_id mime_type
             , b.value -- blob_conten
             , d.document_id --file_nam
             , DBMS_LOB.GETLENGTH(b.value)
          INTO v_mime
              ,lob_loc
              ,v_file_name
              ,v_length
         ...
         WHERE d.stored_document    = B.blob_code
           ...
           AND d.db_office_id       = p_db_office_id;
            ...
            -- application/octect may launch a download window from window
...
--        htp.p('Content-Disposition:  attachment; filename="'||replace(replace(substr(v_file_name,instr(v_file_name,'/')+1),chr(10),null),chr(13),null)|| '"');
   ...
   EXCEPTION
    WHEN no_data_found THEN
     htp.p('No data found for document ' || p_document_code || ' and office ' || p_db_Office_id || '.' );
    
    WHEN others THEN
     htp.p('SQLERRM ' || SQLERRM || ' for for document ' || p_document_code || ' and office ' || p_db_Office_id || '.' );
