...
begin
    ...
    begin
        ...
        insert into at_fcst_location values (l_fcst_spec_code, l_location_code, -1);
...
--               dbms_output.put_line('==> i,j,k,m = '||j||','||j||','||k||','||m||': Setting session to allow use of new LRTS format to '||substr('FT', m, 1));
   ...
   cwms_ts.delete_ts_group(p_ts_category_id => 'TestCategory', p_ts_group_id => 'TestGroup_old', p_db_office_id => c_office_id);
   --.... cwms_cat package
   cwms_cat.cat_ts_aliases(
      p_cwms_cat     => l_crsr,
   ...
   cwms_ts.delete_ts_group(p_ts_category_id => 'TestCategory', p_ts_group_id => 'TestGroup_new', p_db_office_id => c_office_id);
   --.... cwms_cat package
   cwms_cat.cat_ts_aliases(
      p_cwms_cat     => l_crsr,
