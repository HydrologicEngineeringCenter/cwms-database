...
begin
   ...
   dbms_output.put_line('READS : 1='||l_reads(1)||chr(9)||', 2='||l_reads(2)||chr(9)||', Ratio='||case when l_gets(2) = 0 then '???' else to_char(round(l_reads(1)/l_reads(2), 2)) end);
   dbms_output.put_line('TIMES : 1='||l_times(1)||chr(9)||', 2='||l_times(2)||chr(9)||', Ratio='||case when l_gets(2) = 0 then '???' else to_char(round(l_times(1)/l_times(2), 2)) end);
