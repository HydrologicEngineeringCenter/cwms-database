...
--%test(Test group- and filter-based dissemenation routnies)
...
is
   ...
   l_public_name cwms_v_loc.public_name%type;
   l_office_id   cwms_v_ts_id.db_office_id%type;
   l_office_code cwms_v_ts_id.db_office_code%type;
   l_dest_db     varchar2(16);
   function in_group(p_group_id in varchar2) return boolean
   is
      l_count   pls_integer;
   begin
      select count(*)
   ...
   for i in 1..c_filtering_destinations.count loop
      ...
      for j in 1..c_in_group.count loop
         ...
         for k in 1..c_group_ids.count loop
            ...
            commit;
            dbms_output.put(case when in_group(c_group_ids(k)) then 'T' else 'F' end);
         end loop;
         l_dest_db := null;
         cwms_data_dissem.cat_ts_transfer(l_crsr, c_office_id);
         fetch l_crsr into l_cwms_ts_id, l_public_name, l_office_id, l_ts_code, l_office_code, l_dest_db;
         close l_crsr;
         dbms_output.put(' ');
         ...
         dbms_output.put(' ');
         dbms_output.put(cwms_data_dissem.allowed_dest(l_ts_code));
         dbms_output.put(' ');
         dbms_output.put(l_dest_db);
         dbms_output.put_line(null);
         case
         when not cwms_data_dissem.is_filtering_to('DMZ', c_office_id) then
            ...
            ut.expect(l_dest_db).to_equal(c_destinations(cwms_data_dissem.allowed_dest(l_ts_code)));
         when not cwms_data_dissem.is_filtering_to('Corpsnet', c_office_id) then
            ...
            ------------------------------------
            if in_group(c_external_include_group_id) and not in_group(c_external_exclude_group_id) and not in_group(c_internal_exclude_group_id) then
               ut.expect(cwms_data_dissem.allowed_to_dmz(l_ts_code)).to_be_true;
               ut.expect(cwms_data_dissem.allowed_dest(l_ts_code)).to_equal(2);
            else
            ...
            -------------------------------------------------
            if in_group(c_internal_include_group_id) and not in_group(c_internal_exclude_group_id) then
               ut.expect(cwms_data_dissem.allowed_to_corpsnet(l_ts_code)).to_be_true;
               if in_group(c_external_include_group_id) and not in_group(c_external_exclude_group_id) then
                  ut.expect(cwms_data_dissem.allowed_to_dmz(l_ts_code)).to_be_true;
                  ut.expect(cwms_data_dissem.allowed_dest(l_ts_code)).to_equal(2);
               else
                  ut.expect(cwms_data_dissem.allowed_to_dmz(l_ts_code)).to_be_false;
               ...
               if in_group(c_external_include_group_id) and not in_group(c_external_exclude_group_id) and not in_group(c_internal_exclude_group_id) then
                  ...
                  ut.expect(l_dest_db).to_be_null;
               end if;
            end if;
         end case;
      end loop;
