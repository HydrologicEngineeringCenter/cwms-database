...
re_complex_intvl    = r'interval\s+(year\s+to\s+month|day\s+to\s+second)'
...
re_table_type       = r'type\s+('+re_compound_id+r')(\s+'+re_object_id+r')?\s+[ai]s\s+table\s+of\s+('+re_identifier_sized+r')\s*;?'
...
def add_routine(summary_table, details_div, routine_name, routine_type, return_type, params, jdoc_text, local_types=[], local_names=[]) :
   ...
   if param_count == 0 :
      routine_table = HtmlElem('table', attrs=[('class', 'members'+dep)], content= [
         HtmlElem('tr', attrs=[('class', 'inline'+dep)], content=[
            HtmlElem('td', [
               HtmlElem('span', attrs=[('class', 'routine-name'+dep)], content=format(routine_name, False))])])])
   else :
      for i in range(param_count) :
         ...
         if i == 0 :
            routine_table = HtmlElem('table', attrs=[('class', 'members'+dep)], content= [
               HtmlElem('tr', attrs=[('class', 'inline'+dep)], content=[col1, col2, col3, col4])])
         else :
            routine_table.add_content([HtmlElem('tr', attrs=[('class', 'inline'+dep)], content=[col1, col2, col3, col4])])
