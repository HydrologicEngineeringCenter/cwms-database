 ...
 */
   ...
   begin
      if instr(p_expr, '(') > 0 then
         ...
         if l_tokens.count > 0 and
            not is_expression_operator(l_tokens(l_tokens.count)) and
            not is_expression_function(l_tokens(l_tokens.count))
         then
            -----------------------------------------------------------------
      ...
      for i in 1..p_RPN_tokens.count loop  
         ...
         case
            ...
            when p_RPN_tokens(i) = 'NEG' then
               push(pop * -1);
            when p_RPN_tokens(i) = 'ROUND' then
               push(round(pop));
