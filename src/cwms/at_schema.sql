...
(
   ...
   SOURCE_AGENCY        VARCHAR2(5) NOT NULL,
   SOURCE_OFFICE        VARCHAR2(5) NOT NULL,
...
COMMENT ON TABLE  AT_FORECAST_SPEC                      IS 'Contains forecast information for specific locations.';
COMMENT ON COLUMN AT_FORECAST_SPEC.TARGET_LOCATION_CODE IS 'References location.';
COMMENT ON COLUMN AT_FORECAST_SPEC.SOURCE_LOCATION_CODE IS 'Reference to location that is the source of forecats for this location.';
...
ALTER TABLE AT_FORECAST_SPEC ADD CONSTRAINT AT_FORECAST_SPEC_PK  PRIMARY KEY (TARGET_LOCATION_CODE) USING INDEX;
ALTER TABLE AT_FORECAST_SPEC ADD CONSTRAINT AT_FORECAST_SPEC_CK1 CHECK (SOURCE_AGENCY = 'USACE' OR SOURCE_AGENCY = 'NWS');
ALTER TABLE AT_FORECAST_SPEC ADD CONSTRAINT AT_FORECAST_SPEC_CK2 CHECK (UPPER(TRIM(SOURCE_OFFICE)) = SOURCE_OFFICE);
ALTER TABLE AT_FORECAST_SPEC ADD CONSTRAINT AT_FORECAST_SPEC_CK3 CHECK (UPPER(TRIM(FORECAST_TYPE)) = FORECAST_TYPE);
...
CREATE UNIQUE INDEX AT_FORECAST_SPEC_NDX1 ON AT_FORECAST_SPEC(
   TARGET_LOCATION_CODE,
   NVL(SOURCE_LOCATION_CODE, -1),
   SOURCE_AGENCY,
   SOURCE_OFFICE,
   NVL(FORECAST_TYPE, '@'));
