...
CREATE UNIQUE INDEX AT_PHYSICAL_LOCATION_U1 ON AT_PHYSICAL_LOCATION (LOCATION_CODE, UPPER(SUB_LOCATION_ID))
LOGGING
TABLESPACE CWMS_20AT_DATA
PCTFREE    10
INITRANS   2
...
(
   ...
   INTERVAL_IN_LOCAL_TZ    VARCHAR2(1) DEFAULT 'T',
...
COMMENT ON COLUMN AT_LOCATION_LEVEL.PARAMETER_CODE         IS 'References parameter.';
...
COMMENT ON COLUMN AT_LOCATION_LEVEL.LOCATION_LEVEL_COMMENT IS 'Optional comment.';

ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_PK  PRIMARY KEY (LOCATION_LEVEL_CODE) USING INDEX;
ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_U1  UNIQUE (LOCATION_CODE, SPECIFIED_LEVEL_CODE, PARAMETER_CODE, LOCATION_LEVEL_DATE) USING INDEX;
...
ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_CK4 CHECK (INTERVAL_IN_LOCAL_TZ IS NULL OR INTERVAL_IN_LOCAL_TZ IN ('T', 'F'));
ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_CK5 CHECK (INTERPOLATE IS NULL OR INTERPOLATE IN ('T', 'F'));
ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_CK6 CHECK (NOT (LOCATION_LEVEL_VALUE IS NULL AND INTERVAL_IN_LOCAL_TZ IS NULL));
ALTER TABLE AT_LOCATION_LEVEL ADD CONSTRAINT AT_LOCATION_LEVEL_CK7 CHECK (NOT (LOCATION_LEVEL_VALUE IS NULL AND INTERPOLATE IS NULL));
...
(
   LOCATION_LEVEL_CODE NUMBER(10) NOT NULL,
   OFFSET_MONTHS       NUMBER(3)  NOT NULL,
   OFFSET_MINUTES      NUMBER(6)  NOT NULL,
...
COMMENT ON COLUMN AT_SEASONAL_LOCATION_LEVEL.OFFSET_MONTHS       IS 'Months from beginning of period (added to minutes) to this offset.';  
COMMENT ON COLUMN AT_SEASONAL_LOCATION_LEVEL.OFFSET_MINUTES      IS 'Mintues from beginning of period (added to months) to this offset.';
COMMENT ON COLUMN AT_SEASONAL_LOCATION_LEVEL.VALUE               IS 'Seasonal value at this offset in period.';

ALTER TABLE AT_SEASONAL_LOCATION_LEVEL ADD CONSTRAINT AT_SEASONAL_LOCATION_LEVEL_PK  PRIMARY KEY (LOCATION_LEVEL_CODE, OFFSET_MONTHS, OFFSET_MINUTES) USING INDEX;
