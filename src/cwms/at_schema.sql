...
ALTER TABLE AT_LOC_LVL_INDICATOR_COND ADD CONSTRAINT AT_LOC_LVL_INDICATOR_COND_CK4 CHECK (NVL(COMPARISON_VALUE_2, 'EQ') IN ('LT', 'LE', 'EQ', 'NE', 'GE', 'GT'));
...
ALTER TABLE AT_LOC_LVL_INDICATOR_COND ADD CONSTRAINT AT_LOC_LVL_INDICATOR_COND_CK7 CHECK (NVL(RATE_COMPARISON_VALUE_2, 'EQ') IN ('LT', 'LE', 'EQ', 'NE', 'GE', 'GT'));
...
FOR EACH ROW
DECLARE
   L_KIND_ID         AT_LOCATION_KIND.LOCATION_KIND_ID%TYPE;
...
BEGIN
   BEGIN
      SELECT LOCATION_KIND_ID
        INTO L_KIND_ID
        FROM AT_LOCATION_KIND K,
             AT_PHYSICAL_LOCATION  L
       WHERE L.LOCATION_CODE = :NEW.LOCATION_CODE
         AND K.LOCATION_KIND_CODE = L.LOCATION_KIND;
   EXCEPTION
      WHEN NO_DATA_FOUND THEN NULL;         
   END;
   IF L_KIND_ID != 'STREAM' THEN
      SELECT O.OFFICE_ID,
             B.BASE_LOCATION_ID,
             L.SUB_LOCATION_ID
        INTO L_OFFICE_ID,
