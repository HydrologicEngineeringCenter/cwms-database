/* Formatted on 9/9/2011 11:56:55 AM (QP5 v5.163.1008.3004) */
...
AS
    ...
    FUNCTION get_data_stream_code (p_data_stream_id   IN VARCHAR2,
        ...
        RETURN NUMBER
    IS
        l_db_office_id varchar2(16) := cwms_util.get_db_office_id(p_db_office_id => p_db_office_id);
        l_db_office_code     NUMBER
                                     := cwms_util.get_db_office_code (p_db_office_id);
    BEGIN
    begin
    ...
    BEGIN
        ...
        --
        l_data_stream_code := get_data_stream_code(p_data_stream_id => l_data_stream_id, p_db_office_id => l_office_id);
    ...
    IS
        l_update_idle_period   NUMBER := NVL (p_idle_period, 0);
        l_count                      NUMBER;
    BEGIN
        SELECT    COUNT (*)
          INTO    l_count
        ...
        THEN
            ...
            VALUES    (p_db_office_code, l_update_idle_period);
        ELSE
            UPDATE    at_data_stream_properties
                SET    update_idle_period = l_update_idle_period
             WHERE    db_office_code = p_db_office_code;
        END IF;
    END;

    PROCEDURE set_ds_update_idle_period (
        p_idle_period      IN NUMBER DEFAULT 0,
        p_db_office_id   IN VARCHAR2 DEFAULT NULL
    )
    IS
        l_db_office_code     NUMBER
            := cwms_util.get_db_office_code (p_office_id => p_db_office_id);
    BEGIN
        set_ds_update_idle_period (p_idle_period         => p_idle_period,
                                            p_db_office_code     => l_db_office_code
                                          );
    END;
    ...
    IS
        l_db_office_code     NUMBER
            := cwms_util.get_db_office_code (p_office_id => p_db_office_id);
    BEGIN
        set_ds_update_time (p_update_time        => p_update_time,
    ...
    IS
        l_update_idle_period   NUMBER;
    BEGIN
        FOR stale_data_streams IN (SELECT    data_stream_code,
                                                        NVL (update_idle_period, 0) update_idle_period,
                                                        a.new_crit_file_hash, b.idle_time,
                                              ...
                                              FROM    av_data_streams_current a
                                                        ...
                                                        JOIN at_data_stream_properties c
                                                            USING (db_office_code)
                                             WHERE    crit_file_state = 'STALE')
        LOOP
            NULL; -- select update_idle_period into l_update_idle_perod from at_data_stream_properties where d
        END LOOP;
    END;
END cwms_shef;
/