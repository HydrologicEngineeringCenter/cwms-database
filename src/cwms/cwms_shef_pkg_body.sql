...
AS
   ...
   BEGIN
      cwms_apex.aa1 (
         'l_run_time: ' || TO_CHAR (l_run_time, 'YYYY-MM-DD HH24:MI:SS'));
      ...
      FOR rec
        ...
        LOOP
         ...
         cwms_apex.aa1 (
               'l_run_time: '
            || TO_CHAR (l_run_time, 'YYYY-MM-DD HH24:MI:SS')
            || ' data_stream_code: '
            || rec.data_stream_code);

         CASE
            WHEN rec.idle_time IS NULL       -- Need to initialize time & hash
            THEN
               CASE
               ...
               CASE
                  ...
                  THEN          -- Archive Crit File a specific Time of Day...
                     cwms_apex.aa1 (
                        ...
                        || rec.data_stream_code
                        || ' ToD: rec.idle_time: '
                        || TO_CHAR (rec.IDLE_TIME, 'YYYY-MM-DD HH24:MI:SS'));
                     ...
                     CASE
                        ...
                        THEN                 -- Check if Idle Time Exceeded...
                           ...
                           cwms_apex.aa1 (
                              ...
                              || rec.data_stream_code
                              || ' Idl: rec.idle_time: '
                              || TO_CHAR (rec.IDLE_TIME,
                                          'YYYY-MM-DD HH24:MI:SS')
                              || ' l_idle_interval: '
                              || l_idle_interval
                              || ' rec.update_idle_period: '
                              || rec.update_idle_period);
                           ...
                           THEN
                              l_archive_shef_specs := TRUE;
                           END IF;
                        ELSE                 -- Reset both idle time & hash...
                           reset_time_hash (
                              p_data_stream_code   => rec.data_stream_code,
                              p_time               => l_run_time,
                              p_hash               => rec.new_crit_file_hash);
