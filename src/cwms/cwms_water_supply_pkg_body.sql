...
PROCEDURE cat_water_user_contract(
    ...
    -- defaults to the connected user's office if null
    -- the office id can use sql masks for retrieval of additional offices.
    p_db_office_id_mask IN VARCHAR2 DEFAULT NULL )AS
BEGIN
  NULL;
...
PROCEDURE retrieve_water_users(
    ...
    p_project_location_ref IN location_ref_t )AS
BEGIN
  NULL;
...
PROCEDURE delete_water_user(
    ...
    p_delete_action IN VARCHAR2 DEFAULT cwms_util.delete_key )AS
BEGIN
  NULL;
...
PROCEDURE rename_water_user(
    ...
    p_entity_name_new      IN VARCHAR2 )AS
BEGIN
  NULL;
...
PROCEDURE rename_contract(
    ...
    p_new_contract_name IN VARCHAR2 )AS
BEGIN
  NULL;
END rename_contract;
  --------------------------------------------------------------------------------
...
PROCEDURE get_contract_types(
    ...
    -- defaults to the connected user's office if null
    p_db_office_id IN VARCHAR2 DEFAULT NULL )AS
BEGIN
  NULL;
