...
select db_office_id,
       cwms_ts_id,
       local_tz as time_zone,
       case
         when version_date = date '1111-11-11' then null
         else cwms_util.change_timezone(version_date, 'UTC', local_tz)
       end as version_date,
       cwms_util.change_timezone(min_date, 'UTC', local_tz) as min_date,
       cwms_util.change_timezone(max_date, 'UTC', local_tz) as max_date
  from ( select db_office_id,
                cwms_ts_id,
                ts_code,
                location_code
           from av_cwms_ts_id
       ...
       ( select distinct
                ...
                ( select min(date_time)
                    from av_tsv tsv2
                   where tsv2.ts_code = tsv1.ts_code
                ) as min_date,
       ...
       on tsv.ts_code = tsid.ts_code
/        
--------------------------------------------------------------------------------
create or replace force view av_ts_count
   (  db_office_id,
      cwms_ts_id,
      version_date_utc,
      min_date_utc,
...
select db_office_id,
       ...
       end as version_date_utc,
       min_date as min_date_utc,
       max_date as max_date_utc,
       value_count
  from ( select db_office_id,
                cwms_ts_id,
                ts_code
       ...
       ( select distinct
                ...
                ( select count(*)
                    from av_tsv tsv2
                   where tsv2.ts_code = tsv1.ts_code
                     and tsv2.version_date = tsv1.version_date
                ) as value_count
           from av_tsv tsv1
       ) tsv
       on tsv.ts_code = tsid.ts_code
