...
begin                                                     
   ...
   l_top_level_mask    := cwms_util.normalize_wildcards(upper(nvl(p_top_level_mask, '*')));
    ...
    if cwms_util.is_true(p_include_explicit) then
        if cwms_util.is_true(p_include_implicit) then
            ---------------------------------
            -- explicit and implicit pools --
            ---------------------------------
            open p_cat_cursor for
                select *
                from (--------------------
                         ...
                         from (select o.office_id,
                               ...
                               where pn.pool_name_code = po.pool_name_code
                                 ...
                                 and regexp_like(o.office_id, l_office_id_regex)
                                 and upper(base_location_id
                                   ||substr('.', 1, length(sub_location_id))
                                   ||sub_location_id
                                   ||'.'
                                   ||po.bottom_level) like l_bottom_level_mask escape '\'
                                 and upper(base_location_id
                                   ||substr('.', 1, length(sub_location_id))
                                   ||sub_location_id
                                   ||'.'
                                   ||po.top_level) like l_top_level_mask escape '\'
                                 and upper(pn.pool_name) like l_pool_name_mask escape '\'
                                 and upper(bl.base_location_id||substr('.', 1, length(pl.sub_location_id))||pl.sub_location_id) like l_project_id_mask escape '\'
                               union all
                               select o.office_id,
                                      bl.base_location_id,
                                      pl.sub_location_id,
                               ...
                               where pn.pool_name_code = po.pool_name_code
                                 ...
                                 and upper(base_location_id
                                   ...
                                   ||sub_location_id
                                   ||'.'
                                   ||bottom_level) like l_bottom_level_mask escape '\'
                                 and upper(base_location_id
                                   ||substr('.', 1, length(sub_location_id))
                                   ||sub_location_id
                                   ||'.'
                                   ||top_level) like l_top_level_mask escape '\'
                                 and upper(pn.pool_name) like l_pool_name_mask escape '\'
                                 and upper(bl.base_location_id||substr('.', 1, length(pl.sub_location_id))||pl.sub_location_id) like l_project_id_mask escape '\'
                              )
                         ...
                         from (select o.office_id,
                               ...
                               where pr.project_location_code = ll.location_code
                                 ...
                                 and upper(replace(bl.base_location_id
                                                       ||substr('.', 1, length(pl.sub_location_id))
                                                       ||pl.sub_location_id
                                                       ||'.'||bp.base_parameter_id
                                                       ||'.'||pt.parameter_type_id
                                                       ||'.'||d.duration_id
                                                       ||'.'||sp.specified_level_id, 'Top of ', 'Bottom of ')) like l_bottom_level_mask escape '\'
                                 and upper(bl.base_location_id
                                   ...
                                   ||'.'||pt.parameter_type_id
                                   ||'.'||d.duration_id
                                   ||'.'||sp.specified_level_id) like l_top_level_mask escape '\'
                                 and upper(substr(sp.specified_level_id, 8)) like l_pool_name_mask escape '\'
                                 and upper(bl.base_location_id||substr('.', 1, length(pl.sub_location_id))||pl.sub_location_id) like l_project_id_mask escape '\'
                                 and not exists (select project_code,pool_name_code
                                                   from at_pool
    ...
    elsif cwms_util.is_true(p_include_implicit) then
        -------------------------
        -- implicit pools only --
        -------------------------
        open p_cat_cursor for
