CREATE OR REPLACE package BODY CWMS_20.CWMS_CMA IS
/******************************************************************************
   NAME:       CWMS_CMA_ERDC
   PURPOSE:
...
******************************************************************************/
  ...
  FUNCTION apex_log_error(p_error IN apex_error.t_error)
                                RETURN at_cma_error_log.id%TYPE AS
...
BEGIN
  ...
  --
  INSERT
    INTO at_cma_error_log (message, additional_info, display_location, association_type
...
END apex_log_error;
  ...
  FUNCTION f_get_loc_attribs_by_xy(p_latitude       IN cwms_v_loc.latitude%TYPE
...
<ul> '  ||' 
  ...
  RETURN temp_out;
   ...
   BEGIN
    ...
    FOR x IN (SELECT *
                FROM cwms_v_Loc
               WHERE unit_system = 'EN'
                 AND location_id = f_locatioN_id
             ) LOOP
                    NULL;

                    IF x.loc_active_flag = 'F' THEN
                        temp_out    := temp_out || ' Location is inactive';
                        num_issues  := num_issues + 1;
                    END IF;

                    IF x.latitude = 0 OR x.latitude IS NULL
                       OR
                       x.longitude = 0 OR x.latitude IS NULL THEN
  ...
  BEGIN
     ...
     IF UPPER(f_string_1) = UPPER(f_string_2) THEN
      RETURN '=';
     ELSE
...
FOR x IN (
                ...
                FOR y IN (
                          SELECT *
                            ...
                            CASE UPPER(y.specified_level_id)
                             WHEN 'STREAMBED'               THEN num_ll := num_ll + 1;
                             WHEN 'STREAM BED'              THEN num_ll := num_ll + 1;
                             WHEN 'TOP OF FLOOD'            THEN num_ll := num_ll + 1;
                             WHEN 'TOP OF CONSERVATION'     THEN num_ll := num_ll + 1;
                             WHEN 'BOTTOM OF CONSERVATION'  THEN num_ll := num_ll + 1;
                             WHEN 'TOP OF NORMAL'           THEN num_ll := num_ll + 1;
                             WHEN 'TOP OF INACTIVE'         THEN num_ll := num_ll + 1;
                             WHEN 'BOTTOM OF INACTIVE'      THEN num_ll := num_ll + 1;
                             WHEN 'TOP OF INLET'            THEN num_ll := num_ll + 1;
                             WHEN 'BOTTOM OF INLET'         THEN num_ll := num_ll + 1;
                            ELSE
                             NULL;
                           END CASE;
                          END LOOP;
                        temp_out := temp_out || ' - ' || ' found ' || num_ll || ' known location levels';
              NULL;
             ELSE
              NULL;
            END CASE;
  ...
  RETURN temp_out;
  END;
  FUNCTION f_get_nw_loc_link(f_locatioN_id          IN cwms_v_loc.locatioN_id%TYPE
...
FUNCTION get_ts_max_date_utc_2 (
 ...
 --http://www.orafaq.com/usenet/comp.databases.oracle.misc/2004/09/02/0035.htm
    ...
    RETURN ROUND((f_oracle_date - TO_DATE('19700101','yyyymmdd'))*86400 - TO_NUMBER(SUBSTR(TZ_OFFSET(sessiontimezone),1,3))*3600
                 ,0
                );
--    RETURN (f_oracle_date - TO_DATE('19700101','yyyymmdd'))*86400 ;
 END;

  PROCEDURE copy_ts_code_to_ts_code (p_ts_code_from     IN cwms_v_ts_id.ts_code%TYPE
               ...
               WHERE tsv.ts_code = tsi.ts_code
                 AND tsv.ts_code = p_ts_code_from
                  ...
                  --Set the cwms_t_tsv type to the date/time
                    temp_Value := cwms_t_tsv(x.date_time
                                           , x.value
                                           , x.quality_code
                                             );
                ...
                --Store TS Code
                   cwms_ts.store_ts (
                                        p_cwms_ts_id      => temp_cwms_ts_id_to
...
EXCEPTION
 ...
 num_points_needed      NUMBER;
  ...
  BEGIN
    ...
    CASE
     WHEN temp_interval_id_from = temp_interval_id_to THEN
      --Process normally, same number of data points per hour
      NULL;

     WHEN temp_interval_id_from = '1Hour'
            AND
          temp_interval_id_to   = '30Minutes' THEN
   ...
   --from 6145017 to 1054054
    ...
    FOR x IN (SELECT tsv.*
                   , tsi.unit_id
                   , tsi.db_office_id
                   , TO_DATE(tsv.date_time, 'DD-MON-YYYY HH24:MI') date_time_date
                FROM cwms_v_tsv   tsv
                   , cwms_v_ts_id tsi
               WHERE tsv.ts_code = tsi.ts_code
                 AND tsv.ts_code = p_ts_code_from
               ...
               ORDER BY date_Time ASC
                    ...
                    IF temp_value_last IS NOT NULL AND temp_date_time_LAST IS NOT NULL THEN
                        -- this is NOT the first record, so calculate the running average value
                            IF TO_DATE(x.date_time, 'DD-MON-YYYY') + 1/24 = TO_DATE(temp_date_time_last, 'DD-MON-YYYY') THEN
                                ...
                                --  calculate the missing record BEHIND the current location
                                  ...
                                  CASE num_points_needed
                                   WHEN 1 THEN temp_value_avg := temp_value_avg / 2;
                                        --Set the cwms_t_tsv type to the date/time
                                            temp_Value := cwms_t_tsv(TO_TIMESTAMP(x.date_time_date + .5/24)
                                                                   , temp_value_avg
                    ...
                    IF temp_value_last IS NULL OR temp_date_time_last IS NULL THEN
...
EXCEPTION
 WHEN others then p_out := SQLERRM;
  ...
  PROCEDURE Deconstruct_ts_id (p_ts_code_from     IN cwms_v_ts_id.ts_code%TYPE
...
BEGIN
  ...
  END;
  */
   PROCEDURE Clean_Loc_Metadata_by_xy(p_db_office_id             IN cwms_v_ts_id.db_office_id%TYPE
   ...
   FOR x IN (SELECT l.*
              ...
              WHERE l.db_office_id = p_db_office_id
                ...
                IF p_overwrite_county_yn       = c_app_logic_y AND
                   p_overwrite_nation_id_yn    = c_app_logic_y AND
                   p_overwrite_near_city_yn    = c_app_logic_y AND
                   p_overwrite_state_yn        = c_app_logic_y AND
                   p_overwrite_tz_name_yn      = c_app_logic_y
                ...
                ELSE
                    ...
                    IF p_overwrite_county_yn = c_app_logic_y THEN
                    ...
                    IF p_overwrite_nation_id_yn = c_app_logic_y THEN
                    ...
                    IF p_overwrite_near_city_yn = c_app_logic_y THEN
                    ...
                    IF p_overwrite_state_yn = c_app_logic_y THEN
                    ...
                    IF p_overwrite_tz_name_yn = c_app_logic_y THEN
 ...
 PROCEDURE Load_csv_collection_to_DB (p_collection_name IN apex_collections.collection_name%TYPE
                                    , p_store_rule_code  IN cwms_store_rule.store_rule_code%TYPE
                                      ) IS
 temp_value         CWMS_T_TSV;
 temp_value_array   cwms_t_tsv_array;
 ...
 temp_store_rule    cwms_store_rule.store_rule_id%TYPE;
  ...
  BEGIN
    ...
    FOR x IN (SELECT *
               ...
               WHERE collection_name = p_collection_name
                ...
                --Set the cwms_t_tsv type to the date/time
                temp_Value := cwms_t_tsv(TO_TIMESTAMP(x.d001)
                                       , TO_BINARY_DOUBLE(x.n002)
                                       , 3
                                         );
                --Add the record to the array
                temp_value_array.extend();
                ...
                IF temp_cwms_ts_id IS NULL OR temp_unit_id IS NULL OR temp_db_office_id IS NULL THEN
                SELECT cwms_ts_id
                     , unit_id
                     , db_office_id
  ...
  END;
    ...
    PROCEDURE load_application_constants(p_app_logic_yes                   OUT VARCHAR2
   ...
   BEGIN
    ...
    p_app_PAGE_15_INSTRUCTIONS      := 'Follow the Region Titles for Instructions to upload eGIS Metadata <BR>';
    p_app_LOGIC_HTML5_DATE_MASK     := 'YYYY-MM-DD';
    p_app_DB_OFFICE_ID_CWMS         := 'CWMS';
    p_app_usr_dflt_scope_choice     := 'BY_CHECKS';
    p_app_search_goto_loc_icon      := '<img src="#WORKSPACE_IMAGES#Go-16.png" title="Goto Loc" alt="Today">';
...
*/
  ...
  BEGIN
   ...
   UPDATE cwms_store_rule
      SET store_rule_id  = p_description
        , use_as_default = p_default_tf
--        , sort_order     = p_sort_order
  ...
  END;
  PROCEDURE load_tsc_parallel(p_ts_code_left     IN cwms_v_ts_id.ts_code%TYPE
  ...
  BEGIN
    ...
    temp_date_begin := TO_DATE(TO_CHAR(temp_date_begin, 'DD-MON-YYYY'), 'DD-MON-YYYY');
    ...
    FOR x IN (
                ...
                FOR x IN (SELECT MAX(intervals) max_intervals
                               , MIN(increments) min_increments
                            FROM (SELECT temp_intervals_per_day_r intervals, temp_increment_minutes_r increments FROM dual
                                    UNION ALL
                                  SELECT temp_intervals_per_day_l intervals, temp_increment_minutes_l increments FROM dual
                                 )
                          ) LOOP
  ...
  --Create the collection
     apex_collection.create_or_truncate_collection(p_collection_name => p_new_collection_name);
  ...
  --Using the intervals, make a for loop populate all the records in a collection needed to
    ...
    LOOP
        ...
        l_seq := apex_collection.add_member(p_collection_name => p_new_collection_name
                                           ,p_c010            => 'Loading Collection'
                                           ,p_d001            => temp_row_date_time
                                             );

        temp_row_date_time := temp_row_date_time + (temp_increment_minutes / ( 24 * 60));
  ...
  FOR x IN ( SELECT TO_NUMBER(tsv.value) value
             ...
             END LOOP;
  */
  FOR x IN (SELECT ROUND(TO_NUMBER(tsv.value),6) value
                 , tsv.date_time
                 , ac.seq_id
              FROM (SELECT *
                      FROM cwms_v_tsv
                     WHERE ts_code = p_ts_code_left
                       AND (date_time    >= temp_date_begin AND date_time <= temp_date_end)
                    ) tsv
                 , (SELECT * FROM apex_collections WHERE collection_name = p_new_collection_name) ac
   ...
   FOR x IN (SELECT ROUND(TO_NUMBER(tsv.value), 8) value
                 , tsv.date_time
                 , ac.seq_id
              FROM (SELECT *
                      FROM cwms_v_tsv
  ...
  FOR x IN (SELECT *
             ...
             WHERE collection_name = p_new_collection_name
              ...
              CASE
               ...
               WHEN x.n001 IS NULL AND x.n002 IS NULL THEN
                            apex_collection.update_member_attribute(p_collection_name => p_new_collection_name
                                                                       ,p_seq             => x.seq_id
                                                                       ,p_attr_number     => 10
                                                                       ,p_attr_value      => 'No data'
                                                                       );
               WHEN x.n001 = x.n002 THEN
                            apex_collection.update_member_attribute(p_collection_name => p_new_collection_name
                                                                       ,p_seq             => x.seq_id
                                                                       ,p_attr_number     => 10
                                                                       ,p_attr_value      => 'Match'
               ...
               WHEN x.n001 IS NULL AND x.n002 IS NOT NULL THEN
               apex_collection.update_member_attribute(p_collection_name => p_new_collection_name
                                                                       ,p_seq             => x.seq_id
                                                                       ,p_attr_number     => 10
                                                                       ,p_attr_value      => 'Left Side is NULL'
                                                                       );
                WHEN x.n002 IS NULL AND x.n001 IS NOT NULL THEN
               apex_collection.update_member_attribute(p_collection_name => p_new_collection_name
                                                                       ,p_seq             => x.seq_id
                                                                       ,p_attr_number     => 10
        ...
        CASE l_columnno
         WHEN 1 THEN
          CASE nvl(l_token,'**null**')
           WHEN temp_str_ts_id   THEN ts_code_or_ts_id := temp_str_ts_id;
           WHEN temp_str_ts_code THEN ts_code_or_ts_id := temp_str_ts_code;
          ELSE
           NULL;
          END CASE;
        ...
        CASE l_columnno
         ...
         WHEN 1 THEN
          CASE ts_code_or_ts_id
           WHEN temp_str_ts_id   THEN temp_cwms_ts_id := nvl(l_token,'**null**') ;
           WHEN temp_str_ts_code THEN temp_ts_code    := TO_NUMBER(nvl(l_token,'**null**')) ;
          ELSE
           NULL;
          END CASE;
         ...
         WHEN 2 THEN
               temp_date_time_char := nvl(UPPER(TRIM(l_token)),'**null**') ;


               temp_date_time      := TO_DATE(nvl(UPPER(l_token),'**null**'), 'DD/MON/YYYY HH24:MI');

               IF EXTRACT(YEAR FROM temp_date_time) <= 1925 THEN
         ...
         WHEN 3 THEN temp_value   :=  TO_NUMBER(nvl(l_token,'**null**')) ;

         WHEN 4 THEN temp_unit_id :=  nvl(l_token,'**null**') ;

         WHEN 5 THEN temp_db_office_id := nvl(l_token,'**null**') ;
        ELSE
          NULL;
        END CASE;
...
ELSE
     ...
     IF ts_code_or_ts_id = temp_str_ts_code THEN
       SELECT cwms_ts_id
         INTO temp_cwms_ts_id
         FROM cwms_v_ts_id
     ...
     IF ts_code_or_ts_id = temp_str_ts_id THEN
       SELECT ts_code
         INTO temp_ts_code
         FROM cwms_v_ts_id
...
APEX_COLLECTION.ADD_MEMBER (
                        ...
                        --   ,p_blob001         => temp_blob
                             ,p_c001            => CASE ts_code_or_ts_id
                                                    WHEN temp_str_ts_code THEN
                                                     TO_CHAR(temp_ts_code)
                                                    ELSE temp_cwms_ts_id
                                                   END
...
--tsv variables
  ...
  BEGIN
   ...
   BEGIN
   IF p_store_Rule_code IS NULL THEN
...
PROCEDURE p_delete_location_level(p_location_level_code IN cwms_v_Location_level.locatioN_level_code%TYPE
   ...
   BEGIN
    ...
    FOR x IN (
                ...
                cwms_Loc.store_location (
                                      p_location_id         => x.locatioN_id        -- IN VARCHAR2,
  ...
  END ; --p_clear_a2w_ts_code
   ...
   BEGIN
    ...
    NULL;
   END;
   PROCEDURE p_refresh_a2w_ts_codes(p_db_office_id     IN at_a2w_ts_codes_by_loc.db_office_id%TYPE
    ...
    -- SELECT * FROM a2w_ts_codes_by_loc WHERE db_Office_id = 'LRB'
      ...
      DELETE at_a2w_ts_codes_by_loc
       WHERE CASE
              WHEN p_db_office_id IS NULL
               THEN c_temp_null_case
               ELSE p_db_Office_id
              END
             =
             CASE
     ...
     INSERT INTO at_a2w_ts_codes_by_loc (db_office_id, location_id, date_refreshed)
      ...
      AND CASE
         ...
         AND base_parameter_id          IN ('Elev','Stage','Flow','Precip','Stor')
...
--         AND SUBSTR(base_locatioN_id,1,1) IN ('U')
                    ...
                    FOR y IN (SELECT tsi.*
                                ...
                                FROM (
                                   ...
                                   WHERE tsi.ts_code            = tsv.ts_code
                                     ...
                                     AND tsi.location_id        = x.locatioN_id
                                     AND tsi.net_ts_active_flag = c_cwms_logic_t
                                     AND TSI.base_parameter_id  = c_str_elev
                                     AND (UPPER(tsi.versioN_id)    IN ('OBS', 'REV', 'DCP-REV')
                                                OR
                                          INSTR(UPPER(tsi.version_id), '-REV') > 0
                                                OR
                                          INSTR(UPPER(tsi.version_id), 'DECODES') > 0
                                         )
                                     AND INSTR(UPPER(cwms_ts_id), 'FORECAST') = 0
                                   ...
                                   WHERE tsi.ts_code             = tsv.ts_code
                                     ...
                                     AND tsi.net_ts_active_flag  = c_cwms_logic_t
                                     AND TSI.base_parameter_id   = c_str_elev
                                     AND (UPPER(tsi.versioN_id) NOT IN ('OBS', 'REV', 'DCP-REV')
                                             AND
                                          INSTR(UPPER(tsi.version_id), '-REV') =0
                                             AND
                                          INSTR(UPPER(tsi.version_id), 'DECODES') = 0
                             ...
                             ORDER BY sql_level         ASC
                                    , max_date_time     DESC
                            ) LOOP

                             CASE WHEN y.SQL_level = 1
                                            AND
                                       y.row_num = 1
                                            AND
                                       y.max_date_time > SYSDATE - 5 THEN
                              ...
                              SELECT tsi.*
                                ...
                                FROM (
                                  SELECT tsi.db_office_id
                                   ...
                                   WHERE tsi.ts_code            = tsv.ts_code
                                     ...
                                     AND tsi.net_ts_active_flag = c_cwms_logic_t
                                     AND INSTR(UPPER(cwms_ts_id), 'FORECAST') = 0
                                     AND (INSTR(UPPER(versioN_id), '-REV') > 0
                                                 OR
                                          UPPER(version_id) IN ( 'REV', 'OBS')
                                         )
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                                ...
                                UNION ALL
                                   ...
                                   WHERE tsi.ts_code            = tsv.ts_code
                                     ...
                                     AND tsi.net_ts_active_flag = c_cwms_logic_t
                                     AND INSTR(UPPER(cwms_ts_id), 'FORECAST') = 0
                                     AND (UPPER(version_id) NOT IN ( 'REV', 'OBS'))
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                                          , UPPER(tsi.version_id)
                         ...
                         ORDER BY tsi.sql_level, row_num
                             ...
                             CASE
                              WHEN y.sql_level = 1
                                   ...
                                   y.row_num = 1
                                        AND
                                   y.max_date_time > SYSDATE - temp_num_days_countback THEN
                               ...
                               WHEN y.sql_level = 1
                                   ...
                                   y.row_num = 1
                                        AND
                                   y.max_date_time <= SYSDATE - temp_num_days_countback
                                        AND
                                  fire_update_yn = 'Y' THEN

                                       --Don't add the TS code because the date is too old, but this should be an active TS ID?
                                       tsc_precip := y.ts_code;

                                       temp_notes := temp_notes || temp_delim
                                            || ' -  precip CASE has a Active ' || y.version_id
                                            || ' TS with too-old data (' || y.max_date_time  || ' = ' || ROUND((SYSDATE - y.max_date_time),0)
                                            ||  ' days old, the count back is ' || temp_num_days_countback || ') '
                                            ||  y.cwms_ts_id || ' SQL Level = ' || y.sql_level || ' row number = ' || y.row_Num || ' version = ' || y.version_id;
                             ...
                             ELSE
                                ...
                                IF fire_update_yn = 'Y' THEN

                                 temp_notes := temp_notes || temp_delim
                                            || ' - INVALID precip CASE for ' || y.cwms_ts_id || ' SQL Level = ' || y.sql_level || ' row number = ' || y.row_Num || ' version = ' || y.version_id;
                                END IF;
                    ...
                    FOR y IN (SELECT tsi.*
                                ...
                                FROM (
                                   ...
                                   WHERE tsi.ts_code   = tsv.ts_code
                                     ...
                                     AND UPPER(tsi.versioN_id)    IN ('OBS','DCP-REV','REV')
                                     AND INSTR(UPPER(tsi.cwms_ts_id), 'FORECAST') = 0
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                                          , UPPER(tsi.version_id)
                                   ...
                                   WHERE tsi.ts_code   = tsv.ts_code
                                     ...
                                     AND tsi.net_ts_active_flag = c_cwms_logic_t
                                     AND TSI.base_parameter_id = c_str_stage
                                     AND UPPER(tsi.versioN_id)  NOT IN ('OBS','DCP-REV','REV')
                                     AND INSTR(UPPER(tsi.cwms_ts_id), 'FORECAST') = 0
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                             ...
                             ELSE
                               ...
                               IF fire_update_yn = 'Y' THEN
                                 ...
                                 temp_notes := temp_notes || temp_delim
                                            || ' - INVALID stage  CASE for ' || y.cwms_ts_id || ' SQL Level = ' || y.sql_level || ' row number = ' || y.row_Num || ' version = ' || y.version_id;
                                END IF;
                             END CASE;
                    ...
                    FOR y IN (SELECT tsi.*
                                ...
                                FROM (
                                   ...
                                   WHERE tsi.ts_code   = tsv.ts_code
                                     ...
                                     AND tsi.location_id  = x.locatioN_id
                                     AND TSI.base_parameter_id = 'Flow'
                                     AND tsi.sub_parameter_id  NOT LIKE( 'Inflow%')
                                     AND INSTR(UPPER(tsi.cwms_ts_id), 'FORECAST') = 0
                                     AND UPPER(tsi.versioN_id)  NOT IN ('OBS','DCP-REV', 'REV')
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                             ...
                             CASE WHEN UPPER(y.version_id) IN ('OBS','DCP-REV','REV')
                                            AND
                                       y.sub_parameter_id LIKE ('Inflow%')
                                            AND
                                       y.row_num = 1
                                            AND
                                       y.max_date_time > SYSDATE - temp_num_days_countback THEN
                    ...
                    FOR y IN (SELECT tsi.*
                                ...
                                FROM (
                                   ...
                                   WHERE tsi.ts_code   = tsv.ts_code
                                     ...
                                     AND tsi.location_id  = x.locatioN_id
                                     AND TSI.base_parameter_id = 'Flow'
                                     AND tsi.sub_parameter_id NOT LIKE( 'Outflow%')
                                 ...
                                 CASE WHEN UPPER(y.version_id) IN ('OBS','DCP-REV','REV')
                                                AND
                                           y.sub_parameter_id LIKE ('Outflow%')
                                                AND
                                           y.row_num = 1
                                                AND
                                           y.max_date_time > SYSDATE - temp_num_days_countback THEN
                    ...
                    FOR y IN (SELECT tsi.*
                                ...
                                FROM (
                                   ...
                                   WHERE tsi.ts_code            = tsv.ts_code
                                     ...
                                     AND tsi.versioN_id    IN ('OBS','DCP-REV','REV')
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                                          , tsi.sub_parameter_id
                                   ...
                                   WHERE tsi.ts_code   = tsv.ts_code
                                     ...
                                     AND tsi.versioN_id  NOT IN ('OBS','DCP-REV','REV')
                                   GROUP BY tsi.db_Office_id
                                          , tsi.ts_code
                                          , tsi.cwms_ts_id
                                          , tsi.sub_parameter_id
                                 ...
                                 CASE WHEN UPPER(y.version_id) IN ('OBS','DCP-REV','REV')
                                                AND
                                           y.sub_parameter_id IS NULL
                                                AND
                                           y.row_num = 1
                                                AND
                                           y.max_date_time > SYSDATE - temp_num_days_countback
                                                AND
                                           (INSTR(UPPER(y.cwms_ts_id), 'FLOOD') > 0
                                                    OR
                                            INSTR(UPPER(y.cwms_ts_id), 'FLD') > 0
                                           )
                                      ...
                                      WHEN y.version_id IN ('OBS','DCP-REV','REV')
                                           ...
                                           y.max_date_time > SYSDATE - temp_num_days_countback
                                                AND
                                           (INSTR(UPPER(y.cwms_ts_id), 'DROUGHT') > 0
                                                    OR
                                            INSTR(UPPER(y.cwms_ts_id), 'CONSERVATION') > 0
                                                    OR
                                            INSTR(UPPER(y.cwms_ts_id), 'CONSERVE') > 0
                                           )  THEN
                            ...
                            IF fire_i > 0 THEN
                                ...
                                UPDATE at_a2w_ts_codes_by_loc
                                   SET date_refreshed = SYSDATE
                                     , notes          = 'Found ' || fire_i || ' TS IDs for ' || x.locatioN_id || temp_delim || temp_notes
                                     , display_flag   = 'T'
                            ...
                            ELSE
                                ...
                                FOR y IN (SELECT tsi.cwms_ts_id
                                               , MAX(tsv.date_time) max_date_time
                                             FROM cwms_v_ts_id tsi
                                                , cwms_v_tsv tsv
                                                , (SELECT db_office_id, locatioN_id
                                                     FROM at_a2w_ts_codes_by_loc
                                                    WHERE (ts_code_elev     IS NULL
                                                     ...
                                                     AND locatioN_id        = x.location_id
                                                     AND db_office_id       = x.db_office_id)
                                                    ) tsl
                                            WHERE tsi.ts_code = tsv.ts_code (+)
                                              AND tsi.location_id = tsl.locatioN_id
                                              AND tsi.db_office_id = tsl.db_office_id
                                            GROUP BY tsi.cwms_ts_id
                                            ORDER BY 2 DESC
                                        ) LOOP

                                            IF y.max_date_time IS NULL THEN
                                                loop_i_no_data := Loop_i_no_data + 1;
                                            END IF;

                                            IF y.max_date_time <= SYSDATE - 6 THEN
                                                loop_i_old_data := looP_i_old_data + 1;
                                            END IF;
                                          END LOOP;

                                IF loop_i_no_data > 0 OR loop_i_old_data > 0 THEN
                                 UPDATE at_a2w_ts_codes_by_loc
                                    SET date_refreshed = SYSDATE
                                      , notes          = loop_i_no_data || '/' || loop_i_old_data || ' empty/old TS IDs for ' || x.locatioN_id || temp_delim
                                                        || temp_notes
                                      , display_flag   = 'F'
                                  WHERE db_Office_id   = x.db_Office_id
                                    AND location_id    = x.locatioN_id;
  ...
  PROCEDURE p_sync_cwms_office_from_CM2(p_sync_office_buildings_yn  IN VARCHAR2 DEFAULT c_app_logic_y
     ...
     -- Update the division boundaries by office ID
       ...
       UPDATE cwms_office o
          SET o.shape = (SELECT d.shape
                         ...
                         WHERE div = o.office_Id
                        )
         WHERE office_id IN (SELECT div
  ...
  */
  ...
  PROCEDURE p_sync_cwms_geo_tbls_w_CM2(p_compare_or_sync        IN VARCHAR2 DEFAULT 'COMPARE'
                                      ,p_status                 OUT VARCHAR2
                                      ) IS
...
*/
  ...
  PROCEDURE p_sync_cwms_loc_by_NIDID(p_db_Office_id             IN cwms_v_loc.db_Office_id%TYPE
...
BEGIN
 ...
 --this procedure will synconize an office or location's NIDID aliases
    FOR x IN (SELECT l.*
               ...
               WHERE l.unit_system   = 'EN'
                 ...
                 AND CASE
                      WHEN p_locatioN_id IS NULL THEN c_temp_null_case ELSE p_locatioN_id END
                   =
                     CASE
                      WHEN p_locatioN_id IS NULL THEN c_temp_null_case ELSE l.locatioN_id END
               ORDER BY l.location_id
               ) LOOP

                    IF x.nidid IS NULL THEN
                        --Location doesn't have a NIDID in it
                        temp_clob_loop := temp_clob_loop || temp_delim || loop_i || '. ' ||  x.location_id || ' does not have a NIDID association';
                     NULL;

                    ELSE
                       temp_clob_loop := temp_clob_loop || temp_delim || loop_i || '. ' ||  x.location_id || ' has a NIDID association of ' || x.nidid;

                      num_i := num_i + 1;

                        FOR y IN (SELECT d.*
                                     , c.county_name
                                     , s.state_abbr state_initial
                                 ...
                                 WHERE d.state_id  = s.state_id
                                   ...
                                   AND nid_id = x.nidid
                                    ...
                                    temp_clob_loop := temp_clob_loop || temp_delim || ' - . ' ||  x.location_id || ' has a USACE NID association of ' || x.nidid;

                                       IF y.longitude != x.longitude THEN
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' CWMS longitude ' || x.longitude || ' differs from USACE Inventory of Dams ' || y.longitude || ' by ' || (x.longitude - y.longitude) || ' .';
                                       ELSE
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' longitude Matches';
                                       END IF;

                                       IF y.latitude != x.latitude THEN
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' CWMS latitude ' || x.latitude || ' differs from USACE Inventory of Dams ' || y.latitude || ' by ' || (x.latitude - y.latitude) || ' .';
                                       ELSE
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' latitude Matches';
                                       END IF;

                                       IF UPPER(x.public_name) != UPPER(y.dam_name) THEN
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' CWMS public name ' || x.public_name || ' differs from USACE Inventory of Dams ' || y.dam_name || '.' ;
                                       ELSE
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' Public Names Match';
                                       END IF;

                                       IF UPPER(x.state_initial) != UPPER(y.state_initial) THEN
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' CWMS state abbreviation ' || x.state_initial || ' differs from USACE Inventory of Dams ' || y.state_initial || '.' ;
                                       ELSE
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' States Match';
                                       END IF;

                                       IF UPPER(x.county_name) != UPPER(y.county_name) THEN
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' CWMS county name ' || x.county_name || ' differs from USACE Inventory of Dams ' || y.county_name || '.' ;
                                       ELSE
                                        temp_clob_loop := temP_clob_loop || temp_delim || ' - .. ' || ' Counties Match';
                                       END IF;
                 ...
                 NULL;
                    ...
                    IF temp_clob_loop IS NOT NULL THEN
        ...
        temp_clob := ' Found ' || num_i || ' NIDID associations from ' || loop_i || ' locations.'
                    || temp_delim || temp_clob;
 ...
 END;
PROCEDURE p_sync_cwms_nid_w_CM2(p_compare_or_sync        IN VARCHAR2 DEFAULT 'COMPARE'
                               ,p_status                 OUT VARCHAR2
                               ) IS
BEGIN
...
PROCEDURE p_load_tsv (p_collection_name IN apex_collections.collection_name%TYPE
...
NULL;
EXCEPTION
 WHEN others THEN p_display_out := SQLERRM;

END;


  PROCEDURE p_load_Project_Purpose(p_db_Office_id           IN cwms_v_loc.db_office_id%TYPE
                                  ,p_locatioN_id            IN cwms_v_loc.locatioN_id%TYPE
  ...
  */
                     ...
                     WHERE project_location_code = x.location_code
                       ...
                       AND project_purpose_code IN (
                                                        ...
                                                        SELECT TO_NUMBER(column_value) purpose_code
                                                          FROM TABLE(STR2TBL(REPLACE(p_project_purposes,p_delim,',')))
                                                        );

                    IF p_project_purposes IS NOT NULL THEN

                    --insert new values
                    FOR y IN (SELECT TO_NUMBER(column_value) purpose_code
                                FROM TABLE(STR2TBL(REPLACE(p_project_purposes,p_delim,',')))
                               MINUS
                              SELECT TO_NUMBER(project_purpose_code) purpose_code
                               ...
                               WHERE project_location_code = x.location_code
                                 AND purpose_type          = p_purpose_type
                                    ...
                                    temp_additional_notes := 'Added via CMA on ' || TO_CHAR(SYSDATE, 'DD-MON-YYYY HH24:MI');


                                     INSERT INTO at_project_purpose
                                                (project_locatioN_code, project_purpose_code, purpose_type, additional_notes)
                                    VALUES
                                                ( x.locatioN_code, y.purpose_code, p_purpose_Type , temp_additional_notes);
  ...
  END;
    ...
    PROCEDURE p_preload_ll(p_db_office_id             IN cwms_v_locatioN_level.office_id%TYPE
...
--                        ,p_parameter_id             OUT cwms_v_location_level.parameter_id%TYPE
 ...
 temp_char                  VARCHAR2(1999);
  BEGIN
    ...
    --Determine seasonal vs. constant
      ...
      CASE
       WHEN temp_num = 0 THEN
        --do nothing
         ...
         p_level_unit_id := cwms_util.get_default_units (p_base_parameter_id
                                                       , p_unit_system
                                                        );

       WHEN temp_num = 1 THEN
       ...
       cwms_level.parse_location_level_id(
         ...
         SELECT DISTINCT base_parameter_id
                        , sub_parameter_id
                        , SUBSTR(location_level_id
                                , (LENGTH(location_id)+ 1 + LENGTH(base_parameter_id) + 3 + LENGTH(sub_parameter_id))
                                , INSTR(SUBSTR(location_level_id
                                             ,(LENGTH(locatioN_id) + 1 + LENGTH(base_parameter_id) + 3 + LENGTH(sub_parameter_id))
                                              )
                                       , '.'
                                       ) - 1
      ...
      FOR x IN (SELECT  base_parameter_id
                        , sub_parameter_id
                        , SUBSTR(location_level_id
                                , (LENGTH(location_id)+ 1 + LENGTH(base_parameter_id) + 3 + LENGTH(sub_parameter_id))
                                , INSTR(SUBSTR(location_level_id
  ...
  PROCEDURE p_preload_Project_Purpose(p_db_Office_id           IN cwms_v_loc.db_office_id%TYPE
                                  ,p_locatioN_id            IN cwms_v_loc.locatioN_id%TYPE
                                  ,p_delim                  IN VARCHAR2 DEFAULT ','
                                  ,p_purpose_type     IN VARCHAR2 DEFAULT 'AUTHORIZED'
 ...
 PROCEDURE p_preload_loc_by_station_5(p_db_office_id    IN cwms_v_loc.db_Office_id%TYPE
                                     ,p_locatioN_id     IN cwms_v_loc.locatioN_id%TYPE DEFAULT NULL
...
temp_nws_5_id       VARCHAR2(12);

BEGIN

temp_start_date := TO_DATE(TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI'), 'DD/MON/YYYY HH24:MI');
 ...
 FOR x IN (
                SELECT *
                 ...
                 WHERE db_Office_id = p_db_office_id
                   AND CASE
                         WHEN p_location_id IS NULL THEN 'Hello World' ELSE p_locatioN_id END
                       =
                       CASE
                         WHEN p_location_id IS NULL THEN 'Hello World' ELSE locatioN_id END
                  AND (
                        INSTR(UPPER(locatioN_id), 'ART') = 0
                                AND
                        INSTR(UPPER(locatioN_id), 'TEST') = 0
                      )
                  AND (
                      LENGTH(locatioN_id)   = 5
                             OR
                      LENGTH(sub_LocatioN_id) = 5
                                    OR
          location_id IN (SELECT location_id
            ...
            LOOP
                ...
                IF LENGTH(x.location_id) != 5 THEN
             ...
             NULL;
             temp_clob_loop :=  '-- ' || loop_i ||'. Processing NWS Handbook 5 ID ' || temp_nws_5_id || ' for Location "' || x.locatioN_id || '"' || temp_delim;
              ...
              SELECT COUNT(nws_id)
                 ...
                 temp_clob_loop := temp_clob_loop
                                || '-- - Found ' || temp_num || ' NWS Station IDs ' || temp_delim;

                CASE WHEN temp_num = 1 THEN
                    ...
                    FOR y IN (SELECT nws_id
                              ...
                              WHERE nws_id = temp_nws_5_id
                                    ...
                                    p_preload_squery_by_xy(p_lat            => y.lat
                                                          ,p_lon            => y.lon
                                                          ,p_county         => temp_county
                                                          ,p_nation_id      => temp_natioN_id
                                                          ,p_nearest_city   => temp_nearest_city
                                    ...
                                    IF INSTR(temp_nearest_city, '''') > 0 THEN temp_nearest_city := REPLACE(temp_nearest_city, '''', ''''''); END IF;

                                   temp_clob_loop := temp_clob_loop
                                || '-- - Setting CWMS Alias "CWMS Agency Aliases" for "NWS Handbook 5 ID" =  "' || y.nws_id || '" NWS Station IDs ' || temp_delim
                                ||  'cwms_loc.assign_loc_group (
                                   ...
                                   temp_clob_debug_loop := temp_clob_debug_loop
                                || 'CWMS NWS Handbook 5 Alias Link Found for "' || y.nws_id || '"' || temp_delim;

                                IF p_fire_sql = 'T' THEN
                                   cwms_loc.assign_loc_group (
                                                              p_loc_category_id    => 'Agency Aliases'
                                                              ,p_loc_group_id      => 'NWS Handbook 5 ID'
                                                              ,p_location_id       => x.locatioN_id
                                ...
                                END IF;
                                   ...
                                   temp_clob_loop := temp_clob_loop || temp_delim
                                || '-- - Setting CWMS Location Attributes for ' || x.location_id || temp_delim
                                ||  'cwms_loc.update_location2 (
                                         p_location_id        => ''' || x.location_id       || '''
                               ...
                               temp_clob_debug_loop := temp_clob_debug_loop || temp_delim
                                || 'Location attributes in NWS vs. CWMS are:  <BR>
                                      ' || f_validate_string('NAD83'           , x.long_Name     ) ||'    horizontal_datum  => ' || 'NAD83'             || ' vs. ' || x.horizontal_datum ||'<BR>
                                      ' || f_validate_string(x.latitude        , y.lat           ) ||'   latitude          => ' || y.lat               || ' vs. ' || x.latitude         ||'<BR>
                                      ' || f_validate_string(x.longitude       , y.lon           ) ||'   longitude         => ' || y.lon               || ' vs. ' || x.longitude        ||'<BR>
                                      ' || f_validate_string(x.time_zone_name  , temp_time_zone_name) ||'   time_zone_id      => ' || temp_time_zone_name || ' vs. ' || x.time_zone_name   ||'<BR>
                                      ' || f_validate_string(x.db_office_id    , x.db_office_id   ) ||'   db_office_id      => ' || x.db_office_id      || ' vs. ' || x.db_office_id     ||'<BR>
                                      ' || f_validate_string(x.state_initial,temp_state_initial) ||'   state_initial     => ' || temp_state_initial  || ' vs. ' || x.state_initial    ||'<BR>
                                      ' || f_validate_string(x.nation_id    , temp_nation_id   ) ||'   nation_id         => ' || temp_nation_id      || ' vs. ' || x.nation_id        ||'<BR>
                                      ' || f_validate_string(x.county_name       , temp_county      ) ||'   county_name       => ' || temp_county         || ' vs. ' || x.county_name      ||'<BR>
                                      ' || f_validate_string(x.nearest_city , temp_nearest_city) ||'   nearest_city      => ' || temp_nearest_city   || ' vs. ' || x.nearest_city     ||'<BR>
                                      ' || f_validate_string(x.public_Name  , temp_public_Name ) ||'   public_name       => ' || temp_public_Name    || ' vs. ' || x.public_name      ||'<BR>
                                      ' || f_validate_string(x.long_Name       , y.nws_name       ) ||'   long_Name         => ' || y.nws_name          || ' vs. ' || x.long_name        ||'<BR>
                                                )"';
                ...
                ELSE
                    ...
                    IF temp_clob_nws_not_found IS NULL THEN
                        temp_clob_nws_not_found     := temp_clob_nws_not_found  || '''' || x.locatioN_id || '''';
                    ELSE
                        temp_clob_nws_not_found     := temp_clob_nws_not_found  || ', ''' || x.locatioN_id || '''';
                    END IF;
             ...
             IF temp_clob_loop IS NOT NULL THEN
                    temp_clob   := temp_clob || temp_delim || temp_clob_loop || temp_delim;
             END IF;


             IF temp_clob_debug_loop IS NOT NULL THEN
                    temp_clob_debug   := temp_clob_debug || temp_delim || temp_clob_debug_loop || temp_delim;
             END IF;
...
CASE p_debug_or_plsql
 WHEN 'DEBUG' THEN
   p_out := '/* Processed for ' || p_db_office_id || ' on ' || TO_CHAR(temp_start_date, 'DD/MON/YYYY HH24:MI') ||' and found ' || temp_num_found || ' of ' || (loop_i - 1 ) || '. ' || temp_num_not_Found || ' did not have Handbook 5 entries in the NWS Station ID Table USACE_STATION_NWS.'
|| temp_delim || ' CWMS Location ID without Handbook 5 Location IDs include: ' || temp_delim
|| temp_clob_nws_not_found || temp_delim
|| '*/' || temp_delim
|| 'BEGIN' || temp_delim
|| temp_clob_debug || temp_delim
|| 'END;' || temp_delim
|| ' -- Finished procedure on ' || TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI') || '. Duration = ' || TO_CHAR(ROUND(((SYSDATE - temp_start_date)* 24 ) , 2)) || ' minutes.'
;
 WHEN 'PLSQL' THEN
    p_out := '/* Processed for ' || p_db_office_id || ' on ' || TO_CHAR(temp_start_date, 'DD/MON/YYYY HH24:MI') ||' and found ' || temp_num_found || ' of ' || (loop_i - 1 ) || '. ' || temp_num_not_Found || ' did not have Handbook 5 entries in the NWS Station ID Table USACE_STATION_NWS.'
|| temp_delim || ' CWMS Location ID without Handbook 5 Location IDs include: ' || temp_delim
...
END;
  ...
  PROCEDURE p_load_loc_by_station_5(p_db_office_id    IN cwms_v_loc.db_office_id%TYPE
                                   ,p_locatioN_id     IN cwms_v_loc.locatioN_id%TYPE DEFAULT NULL
                                  ) IS
  ...
  END LOOP;
   ...
   NULL;
  END;
  PROCEDURE p_preload_squery_by_xy(p_lat            IN cwms_v_loc.latitude%TYPE
  ...
  IF p_nation_id = 'UNITED STATES' THEN
    ...
    FOR x IN (
            SELECT c.city_name
             ...
             WHERE SDO_NN(c.shape
                        , temp_geometry
                        , 'sdo_num_res=2 unit=MILE', 1) = 'TRUE'
               AND ROWNUM <=5
            ) LOOP
               IF x.row_Num = 1 THEN
    ...
    FOR x IN ( SELECT c.state_name, state_fips, state_abbr
                ...
                IF p_state_initial IS NULL THEN
                    NULL;
       ...
       FOR x IN (SELECT c.zone zone_offset
                  ...
                  WHERE SDO_RELATE(c.shape
                    ...
                    SELECT time_zone_name
                      ...
                      FROM mv_time_zone
                     WHERE time_Zone_name IN ('US/Eastern','US/Central','US/Mountain','US/Pacific') --,'Eastern Standard Time','Central Standard Time','Mountain Standard Time','Pacific Standard Time','Coordinated Universal Time')
...
--    SELECT * FROM cwms_v_ts_id
    ...
    FOR x IN (
                ...
                CASE
                 WHEN INSTR(x.lli, 'Bottom of Normal') > 0 THEN temp_BoN_level_val := x.constant_level;
                 WHEN INSTR(x.lli, 'Top of Normal')    > 0 THEN temp_ToN_level_val := x.constant_level;
                 WHEN INSTR(x.lli, 'Top of Flood')     > 0 THEN temp_ToF_level_val := x.constant_level;
                ELSE
                 NULL;
                END CASE;
              END LOOP;
    ...
    FOR x IN (
                SELECT lli.*
                     , CASE
                        WHEN sysdate > da THEN c_app_logic_y ELSE c_app_logic_n
                       END  in_past
                     , TO_CHAR(SYSDATE, 'MM/DD/YYYY') sysdate1
                  FROM (
                         SELECT location_level_id lli
                              , constant_level
                              , calendar_offset
                              , time_offset
                              , ROUND(seasonal_level,2) seasonal_level
                              , CASE WHEN seasonal_level IS NOT NULL THEN
                              TO_DATE((TO_number(SUBSTR(calendar_offset, 4,2)) + 1) || '/' || LPAD(TO_CHAR((TO_NUMBER(SUBSTR(time_offset, 0, 4))+1)),2,0)
                                || '/' || TO_NUMBER(TO_CHAR(SYSDATE, 'YYYY')), 'MM-DD-YYYY')
                              END da
                              --, TO_TIMESTAMP(calendar_offset || '-' || time_offset) t2
                           FROM cwms_v_location_level
                          WHERE locatioN_id = temp_locatioN_id -- INSTR(location_level_id, temp_location_id) > 0
                            AND unit_System = 'EN'
                            AND seasonal_level IS NOT NULL
                            ) lli
              ORDER BY lli.calendar_offset, lli.time_offset
             ) LOOP
              NULL;

                IF (x.in_past = c_app_logic_n AND temp_seasonal_loop_in_past = c_app_logic_y)
                 OR (x.in_past = c_app_logic_y AND temp_seasonal_loop_in_past = c_app_logic_y)
                 THEN
                    -- We're at the right location

                        IF temp_seasonal_loop_Val IS NULL THEN temp_seasonal_loop_Val := x.seasonal_level; END IF;

                    CASE
                     WHEN INSTR(x.lli, 'Bottom of Normal') > 0      THEN temp_BoN_level_val := temp_seasonal_loop_val;
                     WHEN INSTR(x.lli, 'Top of Normal') > 0         THEN temp_ToN_level_val := temp_seasonal_loop_val;
                     WHEN INSTR(x.lli, 'Top of Flood') > 0          THEN temp_ToF_level_val := temp_seasonal_loop_val;
                     NULL;
                    ELSE
                     NULL;
                    END CASE;
        ...
        IF (temp_BoN_level_val > 0 AND temp_bon_level_val IS NOT NULL)
            AND
            (temp_ToN_level_val > 0 AND temp_ToN_level_val IS NOT NULL) THEN

            --Calculate the Conservation Pool Red and Yellow Lines
                --Red
                 temp_cons_red_val := temp_bon_level_val + ((temp_ton_level_val - temp_bon_level_val) * temp_cons_red_pct);
                --Yellow
                 temp_cons_yellow_val := temp_bon_level_val + ((temp_ton_level_val - temp_bon_level_val) * temp_cons_yellow_pct);


            temp_line_drought_orange :='<line value = "'|| TO_CHAR(temp_cons_yellow_val) ||'" color ="Orange" >
              <label enabled="true">
            ...
            temp_line_drought_red :='<line value = "'|| TO_CHAR(temp_cons_red_val) ||'" color ="Red" >
              <label enabled="true">
               <font color = "Orange" size ="8" />
               <format>Drought</format>
              </label>
...
--SELECT * FROM cwms_v_ts_id@cpc_cwms20 WHERE location_code = 20790066
    ...
    FOR x IN (
               ...
               END LOOP;
                ...
                FOR y IN (SELECT tsv.date_time
                               , ROUND(tsv.value * 3.28084,4) value
                               , ORCL_2_UNIX(tsv.date_time) date_time_unix
                               , TO_CHAR(tsv.date_time, 'DD-MON-YYYY HH24:MI') date_time_char
                            FROM cwms_v_Tsv tsv
                           WHERE tsv.ts_code = p_ts_code
                              AND (tsv.date_Time >= p_date_start --SYSDATE - 60
                                      AND
                                   tsv.date_time <= p_date_end
                                   )
--                             AND tsv.date_Time >= TO_DATE(TO_CHAR(SYSDATE,'DD-MON-YYYY')) - p_days

                           ORDER BY Date_time DESC
                         ) LOOP
                            temp_data_clob := temp_Data_clob || '
          <point x="' ||TO_CHAR(y.date_time_unix) ||' " y="' || TO_CHAR(y.value) ||'" >
           <attributes>
             <attribute name="Date"><![CDATA['|| y.date_time_char ||']]></attribute>
           </attributes>
          </point> ';

                            temp_data_clob_no_point := temp_data_clob_no_point || '
          <point x="' ||TO_CHAR(y.date_time_unix) ||' " y="' || TO_CHAR(y.value) ||'" >
...
IF temp_tof_level_Val IS NULL OR temp_tof_level_Val = 0 THEN
 -- DO nothing
 NULL;
ELSE
temp_line_tof_clob :='             <line value = "'|| TO_CHAR(temp_ToF_level_val) ||'" color="Red" dashed="true" hinting="true" caps="None" dash_length="3" >
              <label enabled="true">
...
IF temp_ton_level_Val IS NULL OR temp_ton_level_Val = 0 THEN
 -- DO nothing
 NULL;
ELSE
temp_line_ton_clob :='             <line value = "'|| TO_CHAR(temp_ToN_level_val) ||'" color="Blue" >
              <label enabled="true">
               <font color = "Blue" size = "8"/>
               <format>ToN ' || TO_CHAR(temp_ToN_level_val) || '</format>
              </label>
             </line>';
END IF;
        ...
        ' || temp_data_clob || '         </series>
      </data>' ;

        temp_clob := '<?xml version="1.0" encoding="UTF-8"?>
<anychart>
   <settings>
...
on: {%Date}]]></format>
      ...
      <chart_settings>
        <title enabled = "true" >
          <text>' || temp_cwms_Ts_id || '</text>
        </title>
        <axes>
          <y_axis>
            <title enabled = "false">
              <text>'|| temp_base_parameter_id || ' (' || temp_unit_id || ') </text>
            </title>
            <labels>
              <format>{%Value}{numDecimals:1,decimalSeparator:.,thousandsSeparator: } ' || 'ft' || '</format>
             </labels>
             <labels>
              <format>{%Value}{numDecimals:1,decimalSeparator:.,thousandsSeparator: }</format>
             </labels>
           <axis_markers>
            <lines>' || temp_line_tof_clob || '
                   ' || temp_line_ton_clob || '
             <line value = "'|| TO_CHAR(temp_BoN_level_val) ||'" color ="Blue" >
              <label enabled="true">
               <font color = "Blue" size ="8" />
               <format>BoN '|| TO_CHAR(temp_BoN_level_val) ||'</format>
              </label>
             </line>
...
FOR x IN (SELECT *
           ...
           WHERE db_Office_id = p_db_Office_id
             AND location_id  =p_locatioN_id
                ...
                NULL;
               IF x.ts_code_elev         IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_stage        IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_precip       IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_inflow       IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_outflow      IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_stor_flood   IS NOT NULL THEN temp_i := temp_i + 1; END IF;
               IF x.ts_code_stor_drought IS NOT NULL THEN temp_i := temp_i + 1; END IF;

                 UPDATE at_a2w_ts_codes_by_loc
                    SET date_refreshed    = SYSDATE
                      , notes             = x.notes || CHR(10) || ' updated via CMA on ' || SYSDATE || ' by ' || 'SYSTEM'
                      , num_ts_codes      = temp_i
                  WHERE db_office_id = x.db_office_id
                   AND location_id   = x.location_id;
                ...
                IF temp_i = 0 THEN
                 ...
                 UPDATE at_a2w_ts_codes_by_loc
                    SET date_refreshed    = SYSDATE
                      , notes             = x.notes || CHR(10) || ' updated via CMA on ' || SYSDATE || ' by ' || 'SYSTEM' || '. Set display flag to False because there are no TS IDs selected.'
                      , display_flag      = 'F'
                  WHERE db_office_id = x.db_office_id
                   AND location_id   = x.location_id;
...
FOR x IN (SELECT location_id, db_office_id
           FROM cwms_v_loc
          WHERE db_Office_id = p_db_Office_id
            AND location_id  = p_location_id
            AND unit_system  = 'EN'
            ...
            AND ( locatioN_kind_id IS NULL OR location_kind_id = 'POINT')
           -- AND substr(location_id, 1,1) = 'B'
          ) LOOP
              INSERT INTO at_a2w_ts_codes_by_loc
                    (db_Office_id, location_id, date_refreshed)
                            VALUES
                    (x.db_office_id, x.location_id, SYSDATE);
            END LOOP;
