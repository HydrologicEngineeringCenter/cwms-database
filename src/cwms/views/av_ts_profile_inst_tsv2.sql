...
delete from at_clob where office_code = 53 and id = '/VIEWDOCS/AV_TS_PROFILE_INST_TSV2';
insert into at_clob values (cwms_seq.nextval, 53, '/VIEWDOCS/AV_TS_PROFILE_INST_TSV2', null,
'
/**
 * Time seires values in all valid units for time series profile instances
 *
 * @since Schema 18.1.6
 *
 ...
 * @field value               The time series value for the parameter, in the indicated unit
 * @field unit_id             The unit for the time series value
...
create or replace view av_ts_profile_inst_tsv2 (
   location_code,
   key_parameter_code,
   ts_code,
   office_id,
...
select tpi.location_code,
       ...
       cwms_util.get_parameter_id(tpi.key_parameter_code) as key_parameter,
       tpi.version_id,
       tpi.first_date_time,
       tpi.last_date_time,
       tpi.version_date,
...
begin
	execute immediate 'grant select on av_ts_profile_inst_tsv2 to cwms_user';
exception
...
create or replace public synonym cwms_v_ts_profile_inst_tsv2 for av_ts_profile_inst_tsv2;
